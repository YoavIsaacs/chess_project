...
/* -------------------------------------------------------------------------
 * Menu stage enum — matches the state machine in spec §2.8.
...
typedef enum
{
    ...
    MENU_TOGGLE_EVAL    = 3,
    MENU_STAGE_DONE     = 4   /* Sentinel — not a real stage; triggers RUNNING */
} MENU_Stage;
 ...
 * @return 1 when the menu is complete (settings confirmed through Stage 4),
